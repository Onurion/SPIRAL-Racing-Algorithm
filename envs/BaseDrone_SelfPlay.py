...
class BaseDrone_SelfPlay(gym.Env):
    ...
    def __init__(self,
                 ...
                 num_drones: int=1,
                 num_dumb_drones: int=0,
                 agents: list= None,
                 dumb_agents: list = None,
        ...
        self.DUMB_URDF = self.DRONE_MODEL.value + "_red.urdf"
        ...
        if self.RECORD:
            assert self.record_folder != None
            # self.ONBOARD_IMG_PATH = os.path.join(self.OUTPUT_FOLDER, "recording_" + datetime.now().strftime("%m.%d.%Y_%H.%M.%S"))
            self.ONBOARD_IMG_PATH = os.path.join(self.record_folder, "recording_" + datetime.now().strftime("%m.%d.%Y_%H.%M.%S"))
        ...
        self.all_agents = self.agents + self.dumb_agents
        ...
        if initial_xyzs is None:
            ...
            self.INIT_XYZS = {}
            N_agents = self.NUM_DRONES + self.NUM_DUMB_DRONES

            for i, agent in enumerate(self.all_agents):
                x = (i % N_agents) * 4 * self.L
                y = (i // N_agents) * 4 * self.L
                z = self.COLLISION_H / 2 - self.COLLISION_Z_OFFSET + 0.1
                self.INIT_XYZS[agent] = np.array([x, y, z])
        ...
        if initial_rpys is None:
            ...
            self.INIT_RPYS = {agent:np.zeros(3) for agent in self.all_agents}
    ...
    def render(self,
        ...
        for name in self.all_agents:
            print("[INFO] BaseAviary.render() ——— drone {}".format(name),
                  "——— x {:+06.2f}, y {:+06.2f}, z {:+06.2f}".format(self.pos[name][0], self.pos[name][1], self.pos[name][2]),
                  "——— velocity {:+06.2f}, {:+06.2f}, {:+06.2f}".format(self.vel[name][0], self.vel[name][1], self.vel[name][2]),
                  "——— roll {:+06.2f}, pitch {:+06.2f}, yaw {:+06.2f}".format(self.rpy[name][0]*self.RAD2DEG, self.rpy[name][1]*self.RAD2DEG, self.rpy[name][2]*self.RAD2DEG),
                  "——— angular velocity {:+06.4f}, {:+06.4f}, {:+06.4f} ——— ".format(self.ang_v[name][0], self.ang_v[name][1], self.ang_v[name][2]))
    ...
    def _housekeeping(self, INIT_XYZS):
        ...
        self.pos = {agent: np.zeros(3) for agent in self.all_agents}
        self.quat = {agent: np.zeros(4) for agent in self.all_agents}
        self.rpy = {agent: np.zeros(3) for agent in self.all_agents}
        self.vel = {agent: np.zeros(3) for agent in self.all_agents}
        self.ang_v = {agent: np.zeros(3) for agent in self.all_agents}
        self.last_clipped_action = {agent: np.zeros(4) for agent in self.all_agents}
        ...
        self.DRONE_IDS = {}
        # agent_urdf_map = {
        #     "agent": pkg_resources.resource_filename('gym_pybullet_drones', 'assets/'+self.URDF),
        #     "dumb_agent": pkg_resources.resource_filename('gym_pybullet_drones', 'assets/'+self.DUMB_URDF)
        # }

        assets_path = "assets"  # Adjust this if the assets folder is elsewhere
        
        agent_urdf_map = {
            "agent": os.path.join(assets_path, self.URDF),
            "dumb_agent": os.path.join(assets_path, self.DUMB_URDF)
        }

        for name in self.all_agents:
            agent_type = "agent" if name in self.agents else "dumb_agent"
            self.DRONE_IDS[name] = p.loadURDF(agent_urdf_map[agent_type],
                                            INIT_XYZS[name],
                                            p.getQuaternionFromEuler(self.INIT_RPYS[name]),
                                            flags=p.URDF_USE_INERTIA_FROM_FILE,
                                            physicsClientId=self.CLIENT)
    ...
    def _updateAndStoreKinematicInformation(self):
        ...
        for agent in self.all_agents:
            self.pos[agent], self.quat[agent] = p.getBasePositionAndOrientation(self.DRONE_IDS[agent], physicsClientId=self.CLIENT)
            self.rpy[agent] = p.getEulerFromQuaternion(self.quat[agent])
            self.vel[agent], self.ang_v[agent] = p.getBaseVelocity(self.DRONE_IDS[agent], physicsClientId=self.CLIENT)
    ...
    def _startVideoRecording(self):
        ...
        if self.RECORD and not self.GUI:
            ...
            self.IMG_PATH = os.path.join(self.record_folder, "recording_" + datetime.now().strftime("%m.%d.%Y_%H.%M.%S"), '')
    ...
    def _getDroneStateVector(self,
        ...
        state = np.hstack([self.pos[drone_name], self.quat[drone_name], self.rpy[drone_name],
                           self.vel[drone_name], self.ang_v[drone_name], self.last_clipped_action[drone_name]])
    ...
    def _getDroneImages(self,
        ...
        rot_mat = np.array(p.getMatrixFromQuaternion(self.quat[agent_name])).reshape(3, 3)
        target = np.dot(rot_mat, np.array([1000, 0, 0])) + np.array(self.pos[agent_name])
        DRONE_CAM_VIEW = p.computeViewMatrix(cameraEyePosition=self.pos[agent_name] + np.array([0, 0, self.L]),
                                            cameraTargetPosition=target,
                                            cameraUpVector=[0, 0, 1],
                                            physicsClientId=self.CLIENT)
    ...
    def _getAdjacencyMatrix(self):
        ...
        adjacency_mat = np.identity(len(self.all_agents))
        for i, agent_i in enumerate(self.all_agents):
            for j, agent_j in enumerate(self.all_agents):
                if agent_i == agent_j:
                    continue

                if np.linalg.norm(self.pos[agent_i] - self.pos[agent_j]) < self.NEIGHBOURHOOD_RADIUS:
    ...
    def _physics(self,
        ...
        for i in range(4):
            p.applyExternalForce(self.DRONE_IDS[drone_name],
                                 i,
        ...
        p.applyExternalTorque(self.DRONE_IDS[drone_name],
                              4,
    ...
    def _groundEffect(self,
        ...
        link_states = p.getLinkStates(self.DRONE_IDS[drone_name],
        ...
        if np.abs(self.rpy[drone_name][0]) < np.pi/2 and np.abs(self.rpy[drone_name][1]) < np.pi/2:
            for i in range(4):
                p.applyExternalForce(self.DRONE_IDS[drone_name],
                                     i,
    ...
    def _drag(self,
        ...
        base_rot = np.array(p.getMatrixFromQuaternion(self.quat[drone_name])).reshape(3, 3)
        #### Simple draft model applied to the base/center of mass #
        drag_factors = -1 * self.DRAG_COEFF * np.sum(np.array(2*np.pi*rpm/60))
        drag = np.dot(base_rot.T, drag_factors*np.array(self.vel[drone_name]))
        p.applyExternalForce(self.DRONE_IDS[drone_name],
                             4,
                             forceObj=drag,
                             posObj=[0, 0, 0],
    ...
    def _downwash(self,
        ...
        for agent in self.agents + self.dumb_agents:
            delta_z = self.pos[agent][2] - self.pos[drone_name][2]
            delta_xy = np.linalg.norm(self.pos[agent][:2] - self.pos[drone_name][:2])
            if delta_z > 0 and delta_xy < 10:  # Ignore drones more than 10 meters away
                alpha = self.DW_COEFF_1 * (self.PROP_RADIUS / (4 * delta_z))**2
                beta = self.DW_COEFF_2 * delta_z + self.DW_COEFF_3
                downwash = [0, 0, -alpha * np.exp(-.5 * (delta_xy / beta)**2)]
    ...
    def _parseURDFParameters(self):
        ...
        urdf_file_path = os.path.join("assets", self.URDF)
