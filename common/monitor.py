...
class Monitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    ...
    def step(self, action: ActType) -> Tuple[ObsType, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        observation, dumb_agent_state, reward, dumb_reward, terminated, truncated, info = self.env.step(action)
        self.rewards.append(float(reward))
        self.dumb_rewards.append(float(dumb_reward))
        if terminated or truncated:
            self.needs_reset = True
            ep_rew = sum(self.rewards)
            ep_dumb_rew = sum(self.dumb_rewards)
            ep_len = len(self.rewards)
            ep_info = {"r": round(ep_rew, 6), "d_r": round(ep_dumb_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6)}
        ...
        self.total_steps += 1
        # print ('monitor')
        return observation, dumb_agent_state, reward, dumb_reward, terminated, truncated, info
...
class ResultsWriter:
    ...
    def __init__(
        ...
        self.logger = csv.DictWriter(self.file_handler, fieldnames=("r", "d_r", "l", "t", *extra_keys))
